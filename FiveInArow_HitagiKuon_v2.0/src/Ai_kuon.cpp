#include <stdio.h>
//AI_KUON_VER_0.3.1  修正棋子下满后闪退bug
#include "Ai_kuon.h"


int kuon_evalTable[KUON_NUM_EVALUATE]=
{
...
int kuon_evaluatePos(char ChessBoard[][MAX_COLUMN],int *EvaluateTable,int row,int column,StepNode tstep)
//点评分
{
	...
	for(i=edge.x1;i<=edge.x2;i++)
	{
		for(j=edge.y1;j<=edge.y2;j++)
		{
			...
			if(ChessBoard[i][j]==color_max)
			{
				...
				tscore=kuon_evaluatePos(ChessBoard,EvaluateTable,row,column,tstep);
...
//max-min<阈值  放子在min位置防守
{
	...
	StepNode tstep; 
	PF_EVA_POS eval_p=&kuon_evaluatePos;//用此估值函数
...
_break_s1:
		...
		if(depth == max_depth)
		{
			...
			pnode=getTop_ScoreStack(ps_stack);		
			tscore=eval_b(ChessBoard,
							EvaluateTable,
							bnode.edge,row,column,color_max,
							ai_info->in.evel_extra);
...
StepNode Ai_kuon_play(char ChessBoard[][MAX_COLUMN],
						int row,int column,
						Edge *edge,
						ChessType color_max)//可以当作训练,公用接口
{
	srand(time(0));
	int p=rand()%100;
	if(p<50)
	{
		return Ai_kuon_pos(ChessBoard,kuon_evalTable,row,column,color_max);
	}
	else
	{
		StepNode tstep;
		...
		ai_info.in.evel_extra=NULL;
		ai_info.in.evel_b=&kuon_evaluateBoard;
		return Ai_kuon_gametree(ChessBoard,kuon_evalTable,&ai_info,color_max);
	}
